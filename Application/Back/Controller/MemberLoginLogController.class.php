    ...
    public function editAction()
    {
        ...
        if (IS_POST) {
            if ($model->create()) {// 校验
                ...
                session('message', ['error'=>1, 'errorInfo'=>$model->getError()]);
                session('data', $_POST);
                $this->redirect('edit', ['member_login_log_id'=>I('post.member_login_log_id')]); // 重定向到添加
           }
       } else {
           $this->assign('message', session('message'));
           session('message', null);// 删除该信息
           // 获取当前编辑的内容, 如果是编辑错误,则显示错误的内容, 如果是没有错误, 则显示原始数据内容
           $this->assign('data', is_null(session('data')) ? $model->find(I('get.member_login_log_id')) : session('data'));
           session('data', null);
           // 展示
           $this->display('set');
       }
...
//        将当前的排序方式, 分配到模板中
    ...
    public function multiAction()
    {
        ...
        switch ($operate) {
            case 'delete':
                $model = M('MemberLoginLog');
                $model->where(['member_login_log_id'=>['in', I('post.selected')]])->delete();
                break;
        }
        $this->redirect('list');
    }
